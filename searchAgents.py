...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (state[1] or state[0] == self.corners[0]) and (state[2] or state[0] == self.corners[1]) and (
                state[3] or state[0] == self.corners[2]) and (state[4] or state[0] == self.corners[3])
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty), (state[1] or state[0] == self.corners[0]), (
                        state[2] or state[0] == self.corners[1]), (state[3] or state[0] == self.corners[2]), (
                        state[4] or state[0] == self.corners[3])
